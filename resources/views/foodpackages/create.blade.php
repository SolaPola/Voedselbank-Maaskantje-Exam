...
<body class="bg-gray-100 min-h-screen">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-4">
                <div>
                    ...
                    <p class="text-sm text-gray-600">Voedselpakket Aanmaken</p>
                </div>
    ...
    <main>
        <div class="max-w-3xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
            <div class="mb-6">
                <h2 class="text-3xl font-bold text-gray-900">Nieuw Voedselpakket</h2>
            ...
            <div class="bg-white shadow rounded-lg overflow-hidden border-t-4 border-orange">
                <form method="POST" action="{{ route('foodpackages.store') }}" class="p-8 space-y-6">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="soort_voedselpakket" value="{{ old('soort_voedselpakket') }}"
                                class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-green">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-500 mb-1">Gezinssamenstelling</label>
                            <input type="text" name="gezinssamenstelling" value="{{ old('gezinssamenstelling') }}"
                                class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-green">
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <select name="isactive" class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-green">
                                <option value="1" {{ old('isactive', '1') == '1' ? 'selected' : '' }}>Ja</option>
                                <option value="0" {{ old('isactive') == '0' ? 'selected' : '' }}>Nee</option>
                            </select>
                        </div>
                    </div>
